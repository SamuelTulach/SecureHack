 ...
 */
static void MemoryCopy(void* destination, void* source, const unsigned long long size)
{
...
static UINT8 HookAllocData[1024];
static unsigned long long HookAllocOffset = 0;
...
{
#ifdef _EFI
    if (HookAllocOffset + size > sizeof(HookAllocData))
        return 0;

    void* address = HookAllocData + HookAllocOffset;
    HookAllocOffset += size;
    return address;
#endif
#ifdef _WIN64
#ifdef _KERNEL_MODE
